...
def cwrap(cls):
    '''
    Class decorator for every method, which wraps wbem exception into LMIShell
    ones.
    '''
    def decorator(fn):
        def method(*args, **kwargs):
            try:
                ...
                raise CIMError(e)
            except wbem.ConnectionError as e:
...
def import_wbem_constants():
    '''
    Helper function which brings wbem's contants into this module's namespace.
    '''
    for member in dir(wbem):
        if member.startswith(('CIM_', 'CON_')):
            globals()[member] = getattr(wbem, member)


# ------------------------------------------------------------------------------

import_wbem_constants()
