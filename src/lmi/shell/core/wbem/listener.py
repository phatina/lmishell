...
from lmi.shell.exceptions import ConnectionError
...
class CIMIndicationHandler(ThreadingMixIn, BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        path = self.path
        if path.startswith("CIMListener/"):
            # We got an indication with lmiwbem CIMListener prefix. We are
            # using pywbem now so it's necessary to strip the prefix.
            path = path[12:]
